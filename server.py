    ...
    def do_GET(self):
        ...
        if k in myHandlers.keys():
            ...
            if myNetHandlers[k] is not None and myNetHandlers[k](self):
                return
            elif None != re.search(k + '/webui/*', self.path):
                l_path = self.path.split("/webui/")[1]  # make this smarter or else security issues
                try:
                    with open('webui/' + l_path, "rb") as fp:
                        self.send_response(200)
                        self.end_headers()
                        answer = fp.read()
                    ...
                    self.send_response(400)
                    self.end_headers()
            elif None != re.search(k + '/client/seek/*', self.path):
                self.success()
